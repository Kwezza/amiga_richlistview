...
static BOOL rlv_is_path_break(char c)
{
    return (c == '/' || c == '\\' || c == ':' || c == '_' || c == '-'
            || c == ' ' || c == '\t');
}
...
static VOID rlv_free_cell_frags(RLV_CellWrap *cell)
{
    if (cell == 0) {
        return;
    }
    if (cell->frags != 0) {
        rlv_platform_free(cell->frags);
        cell->frags = 0;
    }
    cell->frag_count = 0;
}
...
VOID rlv_layout_free_wraps(RLV_Control *c)
{
    ...
    if (c->cell_wraps != 0) {
        for (i = 0; i < c->cell_wrap_count; i++) {
            rlv_free_cell_frags(&c->cell_wraps[i]);
        }
        rlv_platform_free(c->cell_wraps);
        c->cell_wraps = 0;
    }
    c->cell_wrap_count = 0;
}

static BOOL rlv_wrap_cell(RLV_Control *c,
                               CONST_STRPTR src,
                               ...
                               RLV_CellWrap *out)
{
    ...
    WORD text_left;
    WORD text_right;
    RLV_Frag stack_frags[RLV_MAX_FRAGS_PER_CELL];
    UWORD count;
    const char *p;
    WORD first_x;
    ...
    RLV_BENCH_COUNT(RLV_BENCH_COUNTER_CELLS_PREPARED);
    ...
    if (wrap_mode == RLV_WRAP_NONE) {
        UWORD full_len = rlv_strlen(src);
        UWORD fitted;
        UWORD w;

        fitted = rlv_fit_chars(c, src, full_len, (UWORD)fit_width);
        w = 0;
        ...
        stack_frags[0].relative_x = rlv_align_x(alignment,
                                                     text_left,
                                                     text_right,
                                                     w);
        count = 1;
        ...
        while (*p != '\0' && count < RLV_MAX_FRAGS_PER_CELL) {
            ...
            if (*p == '\n' || *p == '\r') {
                ...
                if (count == 0) {
                    stack_frags[count].relative_x =
                        rlv_align_x(alignment, text_left, text_right, 0);
                    first_x = stack_frags[count].relative_x;
                } else if (alignment != RLV_CELL_ALIGN_LEFT) {
            ...
            if (fit_width < 1) {
                ...
                RLV_BENCH_COUNT(RLV_BENCH_COUNTER_WRAP_DECISIONS);
                fitted = rlv_fit_chars(c, (CONST_STRPTR)seg_start,
            ...
            if (count == 0) {
                x = rlv_align_x(alignment, text_left, text_right, w);
                first_x = x;
            } else if (alignment != RLV_CELL_ALIGN_LEFT) {
                x = first_x;
            } else {
    ...
    out->frags = (RLV_Frag *)rlv_platform_malloc(
        (size_t)count * sizeof(RLV_Frag));
    if (out->frags == 0) {
        return FALSE;
    }
    memcpy(out->frags, stack_frags, (size_t)count * sizeof(RLV_Frag));
...
BOOL rlv_wrap_prepare(RLV_Control *c)
{
    ...
    {
        RLV_BENCH_DECLARE(bench_wrap);
        RLV_BENCH_BEGIN(RLV_BENCH_PIXEL_WRAP, bench_wrap);

        rlv_layout_free_wraps(c);

        if (c->row_count == 0 || c->column_count == 0) {
            RLV_BENCH_END(RLV_BENCH_PIXEL_WRAP, bench_wrap);
            return TRUE;
        }

        total = c->row_count * (ULONG)c->column_count;
        c->cell_wraps = (RLV_CellWrap *)rlv_platform_malloc(
            (size_t)total * sizeof(RLV_CellWrap));
        if (c->cell_wraps == 0) {
            RLV_BENCH_END(RLV_BENCH_PIXEL_WRAP, bench_wrap);
            return FALSE;
        }
        memset(c->cell_wraps, 0, (size_t)total * sizeof(RLV_CellWrap));
        ...
        for (row = 0; row < c->row_count; row++) {
            for (col = 0; col < c->column_count; col++) {
                ...
                if (!rlv_wrap_cell(c, cell, &c->col_geom[col],
