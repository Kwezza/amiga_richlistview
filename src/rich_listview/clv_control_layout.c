...
static VOID clv_control_layout_free_owned(CLV_Control *c)
{
    if (c == 0) {
        return;
    }
 ...
 */
typedef struct CLV_CtrlLayoutSnapshot
{
    struct Rectangle header_bounds;
    struct Rectangle viewport_bounds;
    CLV_PixelColumn *col_geom;
    WORD *divider_x;
    UWORD divider_count;
    CLV_RowLayout *layout_rows;
    ...
    UWORD line_height;
    BOOL layout_valid;
} CLV_CtrlLayoutSnapshot;

static VOID clv_ctrl_layout_take_snapshot(CLV_Control *c,
                                          CLV_CtrlLayoutSnapshot *snap)
{
    if (c == 0 || snap == 0) {
        return;
    }
...
static VOID clv_ctrl_layout_restore_snapshot(CLV_Control *c,
                                             CLV_CtrlLayoutSnapshot *snap)
{
    if (c == 0 || snap == 0) {
        return;
    ...
    clv_control_layout_free_owned(c);
...
static VOID clv_ctrl_layout_discard_snapshot(CLV_CtrlLayoutSnapshot *snap)
{
    ULONG i;
    ...
    if (snap->cell_wraps != 0) {
        for (i = 0; i < snap->cell_wrap_count; i++) {
            if (snap->cell_wraps[i].frags != 0) {
                clv_platform_free(snap->cell_wraps[i].frags);
                snap->cell_wraps[i].frags = 0;
            }
            snap->cell_wraps[i].frag_count = 0;
        }
        clv_platform_free(snap->cell_wraps);
        snap->cell_wraps = 0;
    }
    if (snap->layout_rows != 0) {
        clv_platform_free(snap->layout_rows);
        snap->layout_rows = 0;
    }
    if (snap->col_geom != 0) {
        clv_platform_free(snap->col_geom);
        snap->col_geom = 0;
    }
    if (snap->divider_x != 0) {
        clv_platform_free(snap->divider_x);
        snap->divider_x = 0;
    }
    snap->cell_wrap_count = 0;
    snap->divider_count = 0;
...
static VOID clv_control_layout_split_bounds(CLV_Control *c)
{
    WORD header_h;
    WORD max_y;
    WORD inner_min_x;
    ...
    frame = (WORD)CLV_CTRL_FRAME_WIDTH;
...
static VOID clv_ctrl_finish_column_text(CLV_PixelColumn *col, WORD inset)
{
    if (col == 0) {
        return;
    }
 ...
 * collapse.
 */
static BOOL clv_control_layout_columns(CLV_Control *c)
{
    UWORD i;
    WORD x;
    WORD w;
    ...
    CLV_BENCH_ADD(CLV_BENCH_COUNTER_COLUMNS_PROCESSED, c->column_count);
    ...
    c->col_geom = (CLV_PixelColumn *)clv_platform_malloc(
        (size_t)c->column_count * sizeof(CLV_PixelColumn));
    if (c->col_geom == 0) {
        return FALSE;
    }
    memset(c->col_geom, 0,
    ...
    div_count = (c->column_count > 1)
                ...
                : 0;
    if (div_count > 0) {
        c->divider_x = (WORD *)clv_platform_malloc(
    ...
    for (i = 0; i < c->column_count; i++) {
        ...
        clv_ctrl_finish_column_text(&c->col_geom[i], inset);
        ...
        if (i + 1 < c->column_count) {
            ...
            x = (WORD)(x + CLV_CTRL_DIVIDER_WIDTH);
        }
    }
    ...
    if (c->col_geom[last].right < content_right
        ...
        clv_ctrl_finish_column_text(&c->col_geom[last], inset);
    }

    return TRUE;
}

static BOOL clv_control_layout_rows(CLV_Control *c)
{
    ULONG i;
    UWORD col;
    LONG top;
    ...
    CLV_BENCH_ADD(CLV_BENCH_COUNTER_LOGICAL_ROWS, c->row_count);
    ...
    if (!clv_control_wrap_prepare(c)) {
        return FALSE;
    }

    c->layout_rows = (CLV_RowLayout *)clv_platform_malloc(
        (size_t)c->row_count * sizeof(CLV_RowLayout));
    if (c->layout_rows == 0) {
        clv_control_layout_free_wraps(c);
        return FALSE;
    }
    ...
    for (i = 0; i < c->row_count; i++) {
        CLV_BENCH_COUNT(CLV_BENCH_COUNTER_ROW_HEIGHT_CALCS);
        max_lines = 1;
        ...
        top += (LONG)total_h;
        if (max_lines > 1) {
            CLV_BENCH_ADD(CLV_BENCH_COUNTER_WRAPPED_CONTINUATION_ROWS,
                          (ULONG)(max_lines - 1));
        }
    }
 ...
 */
static BOOL clv_control_layout_rebuild_fresh(CLV_Control *c)
{
    if (c == 0) {
        return FALSE;
    }

    clv_control_layout_split_bounds(c);

    if (!clv_control_layout_columns(c)) {
        clv_control_layout_free_owned(c);
        c->layout_valid = FALSE;
        return FALSE;
    }
    if (!clv_control_layout_rows(c)) {
        clv_control_layout_free_owned(c);
        c->layout_valid = FALSE;
        return FALSE;
    }
...
BOOL clv_control_layout_rebuild(CLV_Control *c)
{
    CLV_CtrlLayoutSnapshot snap;
    BOOL had_valid;
    CLV_BENCH_DECLARE(bench_column_layout);
    CLV_BENCH_DECLARE(bench_row_height);

    if (c == 0) {
        return FALSE;
    }

    CLV_BENCH_BEGIN(CLV_BENCH_COLUMN_LAYOUT, bench_column_layout);
    CLV_BENCH_BEGIN(CLV_BENCH_ROW_HEIGHT_CALC, bench_row_height);
    ...
    if (!had_valid) {
        ...
        clv_control_layout_free_owned(c);
        {
            BOOL fresh_ok = clv_control_layout_rebuild_fresh(c);
            CLV_BENCH_END(CLV_BENCH_ROW_HEIGHT_CALC, bench_row_height);
            CLV_BENCH_END(CLV_BENCH_COLUMN_LAYOUT, bench_column_layout);
            return fresh_ok;
        }
    }

    memset(&snap, 0, sizeof(snap));
    clv_ctrl_layout_take_snapshot(c, &snap);

    clv_control_layout_split_bounds(c);
    CLV_LOGF("RESIZE proposed viewport=%d,%d-%d,%d",
    ...
    if (!clv_control_layout_columns(c)
        || !clv_control_layout_rows(c)) {
        CLV_LOG("RESIZE fallback/failure reason=layout_prepare");
        clv_ctrl_layout_restore_snapshot(c, &snap);
        CLV_BENCH_END(CLV_BENCH_ROW_HEIGHT_CALC, bench_row_height);
        CLV_BENCH_END(CLV_BENCH_COLUMN_LAYOUT, bench_column_layout);
        return FALSE;
    }
    ...
    c->layout_valid = TRUE;
    clv_ctrl_layout_discard_snapshot(&snap);
    CLV_BENCH_END(CLV_BENCH_ROW_HEIGHT_CALC, bench_row_height);
    CLV_BENCH_END(CLV_BENCH_COLUMN_LAYOUT, bench_column_layout);
    return TRUE;
}
